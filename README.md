...
原项目地址：[https://github.com/hkfires/EmbyProxy](https://github.com/hkfires/EmbyProxy)
...
- **播放统计可视化**：管理面板内置「可视化」页，以图表展示按天趋势、按小时趋势、节点播放占比与客户端播放排行，播放情况一目了然。图表库（Chart.js）随程序本地嵌入，离线可用。
...
管理面板提供「可视化」页（左侧导航 → **可视化**），用图表呈现播放统计数据，所有图表均由程序内置的 Chart.js 本地渲染，无需联网。

当前包含四类图表：

| 图表 | 说明 |
| ---- | ---- |
| 按天趋势 | 折线图，展示所选天数范围内每天的播放次数、会话数和流量（双 Y 轴）。 |
| 按小时趋势 | 柱状 + 折线图，展示近 72 小时的播放次数与流量分布。 |
| 节点播放占比 | 环形图，按节点展示播放次数占比。 |
| 客户端播放排行 | 横向柱状图，展示 Top 10 客户端的播放次数。 |

使用方式：

1. 打开管理面板，点击左侧 **可视化** tab。
2. 通过右上角下拉框选择按天趋势的统计范围（今天 / 近 7 天 / 近 30 天）。
3. 点击 **刷新** 重新拉取数据。

页面顶部的时间范围选择器只对「按天趋势」和「节点 / 客户端排行」生效；「按小时趋势」固定取近 72 小时（底层 `play_buckets` 表仅保留约 3 天数据）。

> **数据说明**
>
> 播放统计来自 `play_stats` 与 `play_buckets` 两张表，维度为「节点 / 客户端（User-Agent）/ 日期 / 小时」。当前不记录媒体名称、用户名和播放时长，因此暂无「热门媒体排行」「用户活跃度」等按媒体或用户的细分图表。
