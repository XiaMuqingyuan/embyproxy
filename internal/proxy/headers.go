...
func setProxyUA(ids *identity.Manager, h http.Header, node storage.Node) {
	...
	if node.Impersonate {
		snap := ids.Snapshot(node.ImpersonateProfile, node.ImpersonateDeviceName)
		ua = snap.UserAgent
	}
	if ua == "" {
		h.Del("User-Agent")
...
func applyIdentity(ids *identity.Manager, h http.Header, node storage.Node) {
	if node.Impersonate {
		ids.ApplyToHeaders(h, node.ImpersonateProfile, node.ImpersonateDeviceName)
	}
}

func applyIdentityToURL(ids *identity.Manager, u *url.URL, headers http.Header, node storage.Node) {
	if node.Impersonate {
		ids.ApplyToURL(u, headers, node.ImpersonateProfile, node.ImpersonateDeviceName)
	}
}
